...
def _tarea_generar(app, asmt_id: int, uid: int, datos: dict):
    """Tarea ejecutada en el ThreadPoolExecutor con app context propio."""
    with app.app_context():
        ...
        try:
            ...
            md_raw = message.content[0].text
            md_limpio = parsear_md_desde_respuesta(md_raw)

            from routes.admin import parsear_md
            parsed = parsear_md(md_limpio)
            if not parsed:
                raise ValueError(f"Parser no encontró ejercicios. MD preview: {md_limpio[:300]}")
...
def crear_assessment(uid):
    ...
    executor.submit(_tarea_generar, app, asmt.id, uid, datos)
...
def reactivar_assessment(uid, aid):
    ...
    if not asmt.md_generado:
        return jsonify({'error': 'Sin MD guardado'}), 400
    Assessment.query.filter_by(usuario_id=uid, activo=True).update({'activo': False})
    from routes.admin import parsear_md
